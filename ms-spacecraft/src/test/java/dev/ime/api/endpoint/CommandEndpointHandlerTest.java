...
class CommandEndpointHandlerTest {
 	...
 	@MockBean
	private CommandServicePort<SpacecraftDto> commandService;
    ...
    private final static String PATH = "/api/v1/spacecrafts";
	private Event event;
	...
	void create_WithValidEntity_ReturnsCreatedEventWithOkStatus() {
		...
		Mockito.when(dtoValidator.validateDto(Mockito.any(SpacecraftDto.class))).thenReturn(Mono.just(dto));
		Mockito.when(commandService.create(Mockito.any(SpacecraftDto.class))).thenReturn(Mono.just(event));
	
	    webTestClient
        .post().uri(PATH)
        .contentType(MediaType.APPLICATION_JSON)
        .accept(MediaType.APPLICATION_JSON)
        .body(Mono.just(dto), SpacecraftDto.class)
        .exchange()
        .expectStatus().isOk()
    ...
    Mockito.verify(dtoValidator).validateDto(Mockito.any(SpacecraftDto.class));
    Mockito.verify(commandService).create(Mockito.any(SpacecraftDto.class));
	...
	void update_WithValidEntity_ReturnsUpdatedEventWithOkStatus() {
		...
		Mockito.when(dtoValidator.validateDto(Mockito.any(SpacecraftDto.class))).thenReturn(Mono.just(dto));
		Mockito.when(commandService.update( Mockito.any(UUID.class), Mockito.any(SpacecraftDto.class))).thenReturn(Mono.just(event));

		webTestClient
		.put().uri(PATH + "/{id}", Collections.singletonMap("id", spacecraftId))
        .contentType(MediaType.APPLICATION_JSON)
        .accept(MediaType.APPLICATION_JSON)
        .body(Mono.just(dto), SpacecraftDto.class)
        .exchange()
	    ...
	    Mockito.verify(dtoValidator).validateDto(Mockito.any(SpacecraftDto.class));
	    Mockito.verify(commandService).update(Mockito.any(UUID.class), Mockito.any(SpacecraftDto.class));
	}
	...
	void deleteById_WithValidId_ReturnsDeletedEventWithOkStatus() {
		...
		webTestClient
		.delete().uri(PATH + "/{id}", Collections.singletonMap("id", spacecraftId))
        .accept(MediaType.APPLICATION_JSON)
        .exchange()
        .expectStatus().isOk()
        .expectBody(Event.class)
